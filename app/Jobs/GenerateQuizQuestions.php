...
class GenerateQuizQuestions implements ShouldQueue
{
    ...
    public function handle(DeepSeekService $deepSeek): void
    {
        ...
        $operation->update([
            ...
            'result' => ['success' => true, 'questions' => $output['questions'], 'source_label' => $this->sourceLabel],
    ...
    {
        AiOperation::whereKey($this->operationId)->update(['status' => AiOperation::STATUS_FAILED, 'error_message' => mb_substr($e?->getMessage() ?? 'Quiz generation failed', 0, 4000), 'failed_at' => now()]);
    }
}
